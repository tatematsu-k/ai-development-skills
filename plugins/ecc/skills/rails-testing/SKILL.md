---
name: rails-testing
description: "Ruby on Rails testing patterns based on official Rails testing guide, t-wada's TDD philosophy, and DHH's testing stance (Rails 8.1+: system tests dropped from generators). Covers Minitest, fixtures, integration tests, and test architecture."
origin: custom
metadata:
  filePattern:
    - "**/test/**/*.rb"
    ...
    - "**/*_spec.rb"
  bashPattern:
    - "bin/rails test|rake test|bundle exec rspec"
---

# Rails Testing

Based on: [Rails Testing Guide](https://guides.rubyonrails.org/testing.html), [t-wada (和田卓人)](https://t-wada.hatenablog.jp/), DHH, and Rails core team.

## When to Activate

- Writing or reviewing Rails tests
- Setting up test infrastructure
- Debugging failing tests
- Implementing TDD in a Rails project
...
- 内部品質（テスタビリティ、変更容易性、理解容易性）と開発速度は**トレードオフではない** — 相互に強化し合う
- 短期的なスピードのために内部品質を犠牲にすると、技術的負債が将来の開発を遅くする
- テストは「将来の自分への投資」

### TDD の本質

- TDD は「テストの手法」ではなく「設計のフィードバック手法」
- テストを先に書くことで、API を利用者の視点から設計できる
- 「後でテストを書く」は危険 — テストなしで書かれたコードはテストしにくい設計になる

### テストピラミッド（Test Size Classification）
...
- **Small（単体テスト）**: 単一プロセス内で完結。高速で決定的。最も多く書く
- **Medium（統合テスト）**: DB やファイルシステムを使用。単一マシン内
- **Large（E2E）**: 制約なし。ブラウザ自動化、外部サービス

**アイスクリームコーンからピラミッドへ**: 最初は E2E が多くても良い。徐々にユニットテストの比率を増やしていく。

### AI 時代のテスト（2025年）

> 「AI コーディングエージェントの時代、自動テストは"ガードレール"として今まで以上に不可欠」
> — t-wada, 2025
...
- [t-wada: 質とスピード](https://speakerdeck.com/twada/quality-and-speed-2022-spring-edition)
- [t-wada: テストピラミッド](https://logmi.jp/tech/articles/330972)
- [t-wada: AI時代のテスト](https://agilejourney.uzabase.com/entry/2023/11/30/103000)
- [DHH: System tests dropped in Rails 8.1](https://prateekcodes.com/dhh-rails-world-2025-keynote-crud-monkeys/)
- [Aaron Patterson: Test selection](https://shopify.engineering/ruby-rails-year-in-review-2022)
